...
public class Red-Black<K extends Comparable<K>, V> implements Iterable<K> {
	...
	private Node add(Node n, K key, V val) {
		...
		if (getColor(n.left) == RED && getColor(n.left.left) == RED) {			// if n.left is RED means it exists i.e. not null hence we need not check n.left
			n = rightRotate(n);
		}
